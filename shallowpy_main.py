...
for ll in range(nt):
  ...
  if do_height_advection:
    ...
    if do_upwind_advection:
      ...
      flux_x[:,:,1:]  = -(sig_fac[:,:,1:]*ho_adv[:,:,:-1]*uo[:,:,1:] + (1.-sig_fac[:,:,1:])*ho_adv[:,:,1:]*uo[:,:,1:]) * masku[:,:,1:]
      sig_fac = 0.5*(1.+np.sign(vo))
      flux_y[:,1:,:]  = -(sig_fac[:,1:,:]*ho_adv[:,:-1,:]*vo[:,1:,:] + (1.-sig_fac[:,1:,:])*ho_adv[:,1:,:]*vo[:,1:,:]) * maskv[:,1:,:]
    else:
      # --- 2nd order central
      flux_x[:,:,1:]  = -0.5*(ho_adv[:,:,1:]+ho_adv[:,:,:-1])*uo[:,:,1:] * masku[:,:,1:]
      flux_y[:,1:,:]  = -0.5*(ho_adv[:,1:,:]+ho_adv[:,:-1,:])*vo[:,1:,:] * maskv[:,1:,:]
    Tho_adv[:,:-1,:-1] = (flux_x[:,:-1,1:]-flux_x[:,:-1,:-1])/dx + (flux_y[:,1:,:-1]-flux_y[:,:-1,:-1])/dy
    #Tho_adv[:,1:-1,1:-1] = -(
    #    ( 0.5*(ho_adv[:,1:-1,2:]+ho_adv[:,1:-1,1:-1])*uo[:,1:-1,2:] - 0.5*(ho_adv[:,1:-1,1:-1]+ho_adv[:,1:-1,:-2])*uo[:,1:-1,1:-1] ) / dx
    #  + ( 0.5*(ho_adv[:,2:,1:-1]+ho_adv[:,1:-1,1:-1])*vo[:,2:,1:-1] - 0.5*(ho_adv[:,1:-1,1:-1]+ho_adv[:,:-2,1:-1])*vo[:,1:-1,1:-1] ) / dy
    #)
  if do_momentum_advection:
    ...
    #)
    Tuo_adv[:,1:-1,1:-1] = -(
        uo[:,1:-1,1:-1]*(uo[:,1:-1,2:]-uo[:,1:-1,:-2])/(2*dx) 
      + 0.25*(vo[:,1:-1,:-2]+vo[:,1:-1,1:-1]+vo[:,2:,1:-1]+vo[:,2:,:-2])*(uo[:,2:,1:-1]-uo[:,:-2,1:-1])/(2*dy)
    )
    Tvo_adv[:,1:-1,1:-1] = -(
        vo[:,1:-1,1:-1]*(vo[:,2:,1:-1]-vo[:,:-2,1:-1])/(2*dy) 
      + 0.25*(uo[:,:-2,1:-1]+uo[:,1:-1,1:-1]+uo[:,1:-1,2:]+uo[:,:-2,2:])*(vo[:,1:-1,2:]-vo[:,1:-1,:-2])/(2*dx)
    )
