...
# ---
hca, hcb = pyic.arrange_axes(2,2, plot_cb=False, asp=1., fig_size_fac=1.5)
ii=-1
...
ax.legend()
ax.set_title(f'upper channel: H0 = {H0*fac_H0}m')
...
ax.plot(xt/1e3, vop[0,iy,:], label='vop')
ax.legend()
ax.set_title(f'lower channel: H0 = {H0}m')
...
xgrav = -np.sqrt(grav*H0/2.)*ds.time + Lx/2.
ax.plot(ds.time, xgrav/1e3, color='k')
ax.set_title(f'ho with H0 = {H0*fac_H0}m')
